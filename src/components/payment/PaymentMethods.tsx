...
export default function PaymentMethods() {
  ...
  const safetyReminders = [
    {
      ...
      description: 'Every PIN is encrypted and never stored in plain text on your device.'
    },
    {
      id: 'backup',
      icon: Wallet,
      title: 'Always have a backup',
      description: 'Keep at least two payment methods so you can still pay if a channel is down.'
    }
  ];
  ...
  const handleDeletePaymentMethod = async (id: string) => {
    ...
    if (error) return toast.error('Unable to process changes at the moment. Please try again later.');
  ...
  const handleSetDefault = async (id: string) => {
    ...
    const { error: clearError } = await supabase.from('payment').update({ is_default: false }).eq('user_id', user.id);
    if (clearError) return toast.error('Unable to process changes at the moment. Please try again later.');
    const { error: setError } = await supabase.from('payment').update({ is_default: true }).eq('id', id);
    if (setError) return toast.error('Unable to process changes at the moment. Please try again later.');
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <Card className="border-slate-200 bg-slate-50">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm text-slate-600">Saved methods</CardTitle>
            <CreditCard className="w-4 h-4 text-amber-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-semibold text-slate-900">{paymentMethods.length}</div>
            <p className="text-xs text-slate-500 mt-1">
              {paymentMethods.length ? 'Ready for one-tap checkout' : 'Add one to start placing orders'}
            </p>
          </CardContent>
        </Card>
        <Card className="border-slate-200">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm text-slate-600">Default method</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-500 mt-1">
              {defaultMethod ? getPaymentTypeLabel(defaultMethod.type) : 'Pick your go-to option'}
            </p>
          </CardContent>
        </Card>
        <Card className="border-slate-200">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm text-slate-600">Stored balance</CardTitle>
            <Wallet className="w-4 h-4 text-green-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-semibold text-slate-900">RM {preferredBalance.toFixed(2)}</div>
            <p className="text-xs text-slate-500 mt-1">
              {hasPreferredBalance
        ...
        <Card>
          <CardContent className="py-12 text-center space-y-6">
            <div>
              ...
              <h2 className="text-slate-900 text-lg mb-2">No payment methods found. Please add one before managing.</h2>
              <p className="text-slate-500">Link your first method to unlock seamless payments across UTM cafeterias.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
              {['Select a payment type', 'Add the last 4 digits or phone', 'Protect it with a PIN'].map((step, index) => (
                <div key={step} className="p-4 rounded-lg bg-slate-50 border border-slate-100">
                  <p className="text-sm text-slate-500 mb-1">Step {index + 1}</p>
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div>
                    <div className="flex flex-wrap gap-2 items-center mb-1">
                      ...
                      <Badge variant="secondary" className="text-xs">{getPaymentTypeLabel(pm.type)}</Badge>
                      {pm.is_default && <Badge className="text-white bg-purple-600"><Star className="w-3 h-3 mr-1" />Default</Badge>}
                    </div>
                    {pm.details && <p className="text-sm text-slate-600">{getFormattedDetails(pm)}</p>}
          ...
          return (
            <Card key={reminder.id} className="border-slate-200">
              <CardContent className="flex gap-4 items-start pt-6">
                ...
                <div>
                  <p className="text-slate-900 font-medium mb-1">{reminder.title}</p>
          ...
          {supportedOptions.map(option => (
            <Card key={option.id} className="border-slate-200">
              <CardContent className="pt-6">
                <p className="text-xs font-medium text-slate-500 mb-2">{option.id.toUpperCase()}</p>
